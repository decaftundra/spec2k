...
class CreateNotificationPiecePartsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('notification_piece_parts', function (Blueprint $table) {
            $table->string('id')->index()->unique();
            $table->string('notification_id')->index();
            $table->foreign('notification_id')->references('id')->on('notifications');
            $table->string('wpsSFI', 50)->nullable();
        	$table->string('wpsPPI')->unique()->nullable();
    ...
    {
        Schema::dropIfExists('notification_piece_parts');
    }
}
