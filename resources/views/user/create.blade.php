        ...
        <div class="col-xs-12 col-sm-8 col-sm-offset-2">
            ...
            <form class="form-horizontal" method="POST" action="{{ route('user.store') }}">
                ...
                <div class="form-group{{ $errors->has('location_id') ? ' has-error' : '' }}">
                    <label for="role" class="col-md-4 control-label">Location</label>
                        ...
                        <select class="form-control" name="role_id" id="role_id">
                            @if(count($roles))
                                @foreach($roles as $value => $name)
                                    <option value="{{ $value }}" {{ old('role_id') == $value ? 'selected' : '' }}>{{ ucwords(str_replace('_', ' ', $name)) }}</option>
                                @endforeach
                            @endif
                        </select>
                ...
                <div class="form-group{{ $errors->has('planner_group') ? ' has-error' : '' }}">
                    ...
                    <div class="col-md-6">
                        <input id="planner_group" type="text" class="form-control" name="planner_group" value="{{ old('planner_group') }}" autofocus>
