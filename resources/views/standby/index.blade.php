        ...
        <div class="col-xs-12">
            ...
            <div class="navbar-form">
                <form action="{{ request()->url() }}" method="GET">
					...
					<a href="{{ route('standby.index', ['reset' => true]) }}" type="button" class="btn btn-info btn-sm"><i class="fas fa-sync-alt"></i> Reset</a>
                </form>  
            </div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover" style="white-space:nowrap;">
                        ...
                        @foreach ($datasets as $dataset)
                            ...
                            <tr class="{{ $rowClass }}">
                                <td>
                                    <button type="button" role="button" title="Resume" class="btn btn-sm btn-success" onclick="removeOnStandby('{{ $dataset->id }}');">
                                        <i class="fas fa-play"></i>
                                    </button>
                                    
                                    <button type="button" role="button" title="Delete" class="btn btn-sm btn-danger" onclick="doDelete('{{ $dataset->id }}');">
            ...
            @else
                <p>No datasets on standby found.</p>
            @endif
        </div>
    </div>
@endsection
...
@push ('footer-scripts')
    ...
    <script>
        function removeOnStandby(id)
        {
            ...
            var url = "{{ route('status.remove-on-standby') }}";
        	...
        	$.ajax({
                ...
                success : function(data){
                    showStatusChangeAlert('removed_on_standby');
                    setTimeout(function(){
